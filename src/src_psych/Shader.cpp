...
Shader::Shader(std::string vertexPath, std::string fragmentPath, std::string geometryPath)
{
    ...
    checkCompileErrors(vertex, "VERTEX", relativeShaderPath + vertexPath);

    // fragment Shader
    fragment = glCreateShader(GL_FRAGMENT_SHADER);
    glShaderSource(fragment, 1, &fShaderCode, NULL);
    glCompileShader(fragment);
    checkCompileErrors(fragment, "FRAGMENT", relativeShaderPath + fragmentPath);
    ...
    if (geometryPath != "0")
    {
        ...
        checkCompileErrors(geometry, "GEOMETRY", relativeShaderPath + geometryPath);
    }
    // shader Program
    ID = glCreateProgram();
