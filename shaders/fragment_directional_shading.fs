...
float ShadowCalculation(vec4 fragPosLightSpace, vec3 normal, vec3 pos , vec3 fragPos)
{
...
vec3 CalcPointLight(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir)
{
    ...
    float shadow = ShadowCalculation(FragPosLightSpace,normal,light.position,fragPos);                      
...
vec3 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir,vec3 fragPos)
{
    ...
    vec3 diffuse = light.color * diff * vec3(texture(material.texture_diffuse1, TexCoords));
    vec3 specular = light.color * spec * vec3(texture(material.texture_specular1, TexCoords));
    float shadow = ShadowCalculation(FragPosLightSpace,normal,light.direction,fragPos);                    
    //return (ambient +diffuse + specular);
    return (ambient +((1-shadow)* (diffuse + specular)));
